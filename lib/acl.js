...
/**
 deleteResource( resource, function(err) )

 Completely deletes a resource from the system, including any 'allows' collections

 @param {String} Resource to be removed
 @param {Function} Callback called when finished.
 @return {Promise} Promise resolved when finished
 */
Acl.prototype.deleteResource = function (resource, cb) {
    contract(arguments)
        .params('string', 'function')
        .params('string')
        .end();
    ...
    return this.backend.getAsync(this.options.buckets.meta, 'roles').then(function (roles) {
        var transaction = _this.backend.begin();
        _this.backend.drop(transaction, allowsBucket(resource));
        roles.forEach(function (role) {
            _this.backend.remove(transaction, _this.options.buckets.resources, role, resource);
        })
        return _this.backend.endAsync(transaction);
 ...
 *
 * Removes a user from the system, by
 *  1. removing the value 'userId' from the 'user' key of the 'meta' bucket
 *  2. removing the key 'userId' from 'users' bucket
 * @param userId
 * @param cb
 * @returns {*}
 */
Acl.prototype.deleteUser = function (userId, cb) {
    contract(arguments)
    ...
    var transaction = _this.backend.begin();
     ...
     _this.backend
        ...
        .remove(transaction, _this.options.buckets.meta, 'users', userId);

    _this.backend

        .del(transaction, _this.options.buckets.users, userId);
